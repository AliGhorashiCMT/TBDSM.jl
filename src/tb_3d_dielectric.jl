function impol_3d(qx::Real, qy::Real, qz::Real, μ::Real, lat::PyCall.PyObject; histogram_width::Real = 10, mesh::Int=10)
    ...
    bzone_voume = abs(dot(b3, cross(b1, b2)))/1000 ## Brillouin zone volume in inverse angstrom cubed
            ...
            for ziter in 1:mesh
                ...
                for dn_band in 1:num_bands
                    for up_band in 1:num_bands
                        ...
                        if ω>0
                            #println(ω)
                            im_pols[round(Int, ω*histogram_width+1)] = im_pols[round(Int, ω*histogram_width+1)] + 2/(2π)^3*π*histogram_width*(f2-f1)*overlap*(1/mesh)^3*bzone_volume
                        end
                    end
                end
            end
...
function realeps_2d(qx::Real, qy::Real, qz::Real, ωs::Real, μ::Real, lat::PyCall.PyObject; histogram_width::Real=10, mesh::Int=100, max_energy_integration::Real=20)
